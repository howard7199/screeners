...
async function init() {
    try {
        ...
        const [screenerRes, tempRes] = await Promise.all([
            fetch(`../backend/output/screener_data.json?t=${t}`),
            fetch(`../backend/output/temperature.json?t=${t}`)
        ]);
...
function updateTemperature() {
    ...
    if (container && data.components) {
        ...
        const names = {
            ...
            'volatility_inverse': '波動率反向',
            'turtle_net': '海龜淨突破',
            'institutional_sentiment': '法人情緒'
        };
...
function populateSelectors() {
    if (!appState.screenerData?.stocks) return;
...
function renderTable() {
    ...
    pageData.forEach((s, idx) => {
        ...
        const yoyStr = s.revenue?.yoy_pct != null ? `${s.revenue.yoy_pct > 0 ? '+' : ''}${s.revenue.yoy_pct}%` : '--';
        const yoyClass = s.revenue?.yoy_pct > 0 ? 'pos' : (s.revenue?.yoy_pct < 0 ? 'neg' : 'neu');
        
        tr.innerHTML = `
            <td>${start + idx + 1}</td>
            <td class="num font-weight-bold">${s.symbol ?? '--'}</td>
            <td>${s.name ?? '--'}</td>
            <td class="num">${formatNum(s.price, 1)}</td>
        ...
        const cols = 13; // updated column count
        
        const momStr = s.revenue?.mom_pct != null ? `${s.revenue.mom_pct > 0 ? '+' : ''}${s.revenue.mom_pct}%` : '--';
        const momClass = s.revenue?.mom_pct > 0 ? 'pos' : (s.revenue?.mom_pct < 0 ? 'neg' : 'neu');
        ...
        trDetail.innerHTML = `
            <td colspan="${cols}">
                <div class="detail-content">
                    ...
                    <div class="detail-group">
                        <h4>營收表現</h4>
                        <div class="detail-item"><span class="lbl">月營收 YoY</span><span class="val ${yoyClass}">${yoyStr}</span></div>
                        <div class="detail-item"><span class="lbl">月營收 MoM</span><span class="val ${momClass}">${momStr}</span></div>
                    </div>
                    <div class="detail-group">
                    ...
                    <div class="detail-group">
                        ...
                        ${s[btKey]?.army ? `
                            <div class="detail-item"><span class="lbl">總報酬率</span><span class="val ${(s[btKey].army?.return_pct ?? 0) > 0 ? 'pos' : 'neg'}">${formatNum(s[btKey].army?.return_pct)}%</span></div>
...
function renderHeatmap() {
    ...
    requestAnimationFrame(() => {
        ...
        if (!container) { renderHeatmap._rendering = false; return; }
        container.innerHTML = '';
        
        if (appState.filteredData.length === 0) { renderHeatmap._rendering = false; return; }
        
        const fragment = document.createDocumentFragment();
        let dataToRender = appState.filteredData.slice(0, 200);
        
        // Group by sector/industry
        const categories = {};
        dataToRender.forEach(s => {
            const cat = s.industry || s.sector || '其他';
        ...
        if (totalValue === 0) { renderHeatmap._rendering = false; return; }
        ...
        catNodes.forEach(cat => {
            ...
            etfs.forEach((s, i) => {
                ...
                node.title = `${s.symbol || s.yf_ticker || '--'} ${s.name || '--'}\n漲跌: ${change > 0 ? '+' : ''}${change.toFixed(2)}%`;
...
function setupEventListeners() {
    ...
    // Selects and Text
    ['timeframe', 'market', 'army', 'macd', 'turtle', 'sector', 'search'].forEach(filter => {
    ...
    if (btnDaily && btnHourly) {
        btnDaily.addEventListener('click', (e) => {
            ...
            const sel = document.getElementById('filter-timeframe');
    ...
    // Quick Filters
    const qfSetup = [
        ['qf-momentum', () => { resetFilters(); appState.filters.momentum = 80; }],
        ['qf-value', () => { resetFilters(); appState.filters.valuation = 70; }],
        ['qf-growth', () => { resetFilters(); appState.filters.quality = 70; appState.filters.growth = 70; }],
        ['qf-army', () => { resetFilters(); appState.filters.army = '多方排列'; }],
...
function resetFilters() {
    appState.filters = {
        ...
        market: 'all', army: 'All', macd: 'All', turtle: 'All', sector: 'all', search: ''
    };
}
...
function updateUI() {
    const f = appState.filters;
    const updates = [
        ['filter-momentum', f.momentum], ['val-momentum', f.momentum, true],
        ['filter-quality', f.quality], ['val-quality', f.quality, true],
        ['filter-valuation', f.valuation], ['val-valuation', f.valuation, true],
        ['filter-growth', f.growth], ['val-growth', f.growth, true],
        ['filter-market', f.market],
        ['filter-army', f.army],
        ['filter-macd', f.macd],
        ['filter-turtle', f.turtle],
        ['filter-sector', f.sector],
...
function generateMockData() {
    const sectors = ['半導體', '金融保險', '電腦及週邊設備', '電子零組件', '航運業', '生技醫療'];
    const markets = ['上市', '上櫃', '興櫃'];
    
    return {
        updated_at: new Date().toISOString(),
        stock_count: 1782,
        stocks: Array.from({length: 500}, (_, i) => {
            const secIdx = Math.floor(Math.random() * sectors.length);
            const mktIdx = Math.floor(Math.random() * markets.length);
            // TW style codes: 4 digits
            const symbol = String(1101 + i).padStart(4, '0');
            return {
                symbol: symbol,
                name: `台股 ${symbol}`,
                market: markets[mktIdx],
                sector: sectors[secIdx],
                sector_zh: sectors[secIdx],
                price: (Math.random() * 1990 + 10).toFixed(1), // 10 to 2000
                change_1d_pct: (Math.random() * 20 - 10).toFixed(2), // -10% to 10%
                market_cap: Math.random() * 1000000000000 + 100000000, // min 1億
                factors: {
                ...
                revenue: {
                    yoy_pct: (Math.random() * 100 - 30).toFixed(1),
                    mom_pct: (Math.random() * 40 - 20).toFixed(1)
                },
                raw: {
                    pe: (Math.random() * 50).toFixed(1),
                    ps: (Math.random() * 10).toFixed(1),
                    eps: (Math.random() * 20 - 2).toFixed(2),
                    roe: (Math.random() * 30 - 5).toFixed(1),
                    net_margin: (Math.random() * 30).toFixed(1),
                    revenue_growth: (Math.random() * 50 - 10).toFixed(1)
                },
                technical_daily: {
                    army: {
...
function generateMockTemp() {
    const gen = () => ({
        ...
        components: {
            ...
            vix_inverse: { value: Math.floor(Math.random() * 101), weight: 15, detail: "波動率 15.99" },
            turtle_net: { value: Math.floor(Math.random() * 101), weight: 10, detail: "突破 13 · 破低 21" },
            pc_ratio_inverse: { value: Math.floor(Math.random() * 101), weight: 10, detail: "法人情緒 0.82" }
        }
    });
    return {
        updated_at: new Date().toISOString(),
